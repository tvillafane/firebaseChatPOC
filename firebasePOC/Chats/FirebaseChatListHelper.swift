...
//        }
    ...
    func listenForNewConversations() {
        ...
        convoRef.observe(.childAdded) { (snapShot) in
            let messageSnap = snapShot.childSnapshot(forPath: Constants.lastMessageKey)

            if let message = Message(snapShot: messageSnap) {
                //let backendId = snapShot.key.replacingOccurrences(of: "c_", with: "", options: .literal, range: nil)
                let convo = Conversation(backendId: snapShot.key, lastMessage: message)
